...
function draw() {
    ...
    snake.forEach((part, index) => {
        if (index === 0) {
            const HEAD_SCALE = 2.1; // ingrandisci come vuoi!
    ...
    const FOOD_SCALE = 1.6;  // aumenta o diminuisci a piacere
    const fsize = gridSize * FOOD_SCALE;
    
    ctx.drawImage(
        foodImg,
        food.x - (fsize - gridSize) / 2,
        food.y - (fsize - gridSize) / 2,
        fsize,
        fsize
    );
